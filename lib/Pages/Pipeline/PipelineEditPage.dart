...
class PipelineEditPage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return _PipelineEditState();
  }
}

class _PipelineEditState extends State<PipelineEditPage> {
  final AddPipelineBloc addPipelineBloc = AddPipelineBloc();

  final _dealNameController = TextEditingController();
  MoneyMaskedTextController _dealAmountController;
...
//purpose to declare future for futurebuilder here is to let futurebuilder use the same instance of future, so it will not keep calling REST API
  ...
  var cogsAndMarginDropdownValue;
  ...
  Widget build(BuildContext context) {
    if (isInit) {
      ...
      selectedPipline = ModalRoute.of(context).settings.arguments;
      _dealAmountController = MoneyMaskedTextController(
          decimalSeparator: '.',
          thousandSeparator: ',',
          initialValue: selectedPipline.dealAmount);
      _dealNameController.text = selectedPipline.dealName;
      selectedPeople = selectedPipline.people;
      selectedCompany = selectedPipline.company;
      selectedStage = selectedPipline.stage;
      typeDropdownValue = selectedPipline.type;
      selectedAttainDateTime = selectedPipline.attainDate;
      _noteController.text = selectedPipline.note;
      if (selectedPipline.cogsAmount != null) {
        cogsAndMarginDropdownValue = 'COGS Amount';
        _cogsAmountController.text = selectedPipline.cogsAmount.toStringAsFixed(2);
        _estimatedGPController.text = (selectedPipline.dealAmount - selectedPipline.cogsAmount).toStringAsFixed(2);
      }
      if (selectedPipline.margin != null) {
        cogsAndMarginDropdownValue = 'Cost Margin%';
        _marginController.text =
            (selectedPipline.margin * 100).toStringAsFixed(0);
            _estimatedGPController.text = (selectedPipline.dealAmount*(1-selectedPipline.margin)).toStringAsFixed(2);
      }
    }
    ...
    return Scaffold(
      ...
      appBar: GeneralAppBar(
              'Edit Deal', 'Deal', formKey, _scaffoldKey, confirmButtonCallback)
          .create(),
      body: pipelineContainer(),
    );
  }
  ...
  void postForm() async {
    selectedPipline.peopleId = selectedPeople?.id;
    selectedPipline.companyId = selectedCompany?.id;

    selectedPipline.stageId = selectedStage?.id;
    // selectedPipline.employeeId = assignedEmployee?.id;
    selectedPipline.dealName = _dealNameController.text;
    if (cogsAndMarginDropdownValue == 'COGS Amount') {
      selectedPipline.cogsAmount =
          double.parse(_cogsAmountController.text.replaceAll(",", ""));
      selectedPipline.margin = null;
    }
    if (cogsAndMarginDropdownValue == 'Cost Margin%') {
      selectedPipline.margin = double.parse(_marginController.text) / 100;
      selectedPipline.cogsAmount = null;
    }
    selectedPipline.dealAmount =
        double.parse(_dealAmountController.text.replaceAll(",", ""));
    selectedPipline.type = typeDropdownValue;
    selectedPipline.attainDate = selectedAttainDateTime;
    selectedPipline.note = _noteController.text;
    ...
    try {
      ...
      await _pipeListBloc.updatePipeline(selectedPipline);
      PipelineListPageState.isInit = true;
      await ShowSnackBarAndGoBackHelper.go(
          _scaffoldKey, "Deal Updated", context);
      //} else {
  ...
  Widget get linkCompanyRow {
    return SetRelationWidget(
      ...
      selectedCompany:
          selectedPeople != null ? selectedPeople.companyId : selectedCompany,
  ...
  Widget get linkPersonRow {
    return SetRelationWidget(
        SetRelationOption.people, onSelectedCallBack, removeSelectedCallBack,
        selectedPeople: selectedPeople,
        noDataDisplay: NoContactInfoGuideWidget(2));
    // return Container(
    //   // height: 50,
    ...
    return CustomStreamBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        ...
        if (snapshot.hasData && snapshot.data != null) {
          ...
          return CustomDropdownSelection("", allStageList, selectedStage,
              (value) {
            setState(() {
              selectedStage = value;
            });
